...
type Props = { state: IState };

const DeleteOrRestoreSingleState = ({ state }: Props) => {
	...
	// server state hooks
	const deleteSingleStateMutation = useDeleteSingleStateMutation();
	const restoreSingleStateMutation = useRestoreSingleStateMutation();

	// ref hook
	const modalRef = useRef<HTMLDivElement | null>(null);
	const deleteOrRestoreStateCancelRequestRef = useRef<AbortController | null>(null);

	// state hooks
	const [isDeleteOrRestoreLoadingState, setIsDeleteOrRestoreLoadingState] = useState(false);

	// constants
	const isStateDeleted = state?.deleted || false;

	// event handlers
	const deleteOrRestoreStateHandler = async () => {
		setIsDeleteOrRestoreLoadingState(true);

		// Abort any previous request, and create a new abort controller.
		if (deleteOrRestoreStateCancelRequestRef.current?.signal)
			deleteOrRestoreStateCancelRequestRef.current?.abort();
		deleteOrRestoreStateCancelRequestRef.current = new AbortController();

		const mutation = isStateDeleted ? restoreSingleStateMutation : deleteSingleStateMutation;
		...
		await mutation.mutateAsync(
			{
				...
				data: {},
				signal: deleteOrRestoreStateCancelRequestRef.current.signal,
			},
			{
				// Reset loading state
	...
	return (
		...
		ReactDOM.createPortal(
			<div
				...
				ref={modalRef}
				tabIndex={-1}
				id={`deleteOrRestoreSingleState${state?._id}Modal`}
				aria-labelledby={`deleteOrRestoreSingleState${state?._id}ModalTitle`}
				aria-hidden="true">
					...
					<div className="modal-content bg-white">
						<div className="modal-header border-0">
							<h5
								...
								id={`deleteOrRestoreSingleState${state?._id}ModalTitle`}>
						...
						<div className="modal-body py-2">
							<p className="text-center mb-0">
								{`Confirming that this action will ${isStateDeleted ? "restore/un-archive" : "delete/archive"} this state?`}
							</p>
						</div>
								...
								<div className="col-12 col-md">
									<button
										...
										className={classNames(
											...
											{
												"btn-primary": isStateDeleted,
												"btn-danger": !isStateDeleted,
											}
										)}
										onClick={() => deleteOrRestoreStateHandler()}
										disabled={isDeleteOrRestoreLoadingState}>
										<strong>{isStateDeleted ? "Restore" : "Delete"}</strong>
